...
function buildChips() {
    ...
    const defs = [["all", "すべて"], ...Object.entries(BRANDS).map(([k, v]) => [k, v.chip])];
...
async function setCheckin(id, on) {
...
async function bulkSetCheckin(pref, on) {
    ...
    for (const s of prefStores) {
        if (!!checkins[s.id] === on)
            continue;
        if (on)
            checkins[s.id] = todayStr();
        else
            delete checkins[s.id];
        changed++;
...
function prefCheckState(pref) {
    const prefStores = STORES.filter(s => s.prefecture === pref);
    const doneCount = prefStores.filter(s => checkins[s.id]).length;
    if (doneCount === 0)
        return "none";
    if (doneCount === prefStores.length)
        return "all";
    return "some";
}
function buildList() {
    ...
    const stores = STORES.filter(s => {
        if (prefFilter !== "all" && s.prefecture !== prefFilter)
            return false;
        if (listFilter === "unvisited")
            return !checkins[s.id];
        if (listFilter === "visited")
            return !!checkins[s.id];
        return true;
    });
    ...
    body.querySelectorAll(".pref-check").forEach(cb => {
        cb.indeterminate = prefCheckState(cb.dataset.pref) === "some";
    });
}
function openList() {
    buildList();
...
document.getElementById("list-body").addEventListener("click", (e) => {
    ...
    if (toggle) {
        ...
        if (expandedPrefs.has(pref))
            expandedPrefs.delete(pref);
        else
            expandedPrefs.add(pref);
        buildList();
        return;
